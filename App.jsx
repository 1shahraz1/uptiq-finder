    ...
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {/* Replace /logo.svg with your actual logo path; falls back to text */}
...
function FinderCard() {
  ...
  const steps = [
    {
      key: "experience",
      q: "What’s your trading experience?",
      options: [
        { v: "new", label: "New to algos" },
        { v: "intermediate", label: "Some experience" },
        { v: "advanced", label: "Advanced / prop" },
      ],
    },
    {
      key: "platform",
      ...
      options: [
        { v: "mt5", label: "MetaTrader 5" },
        { v: "ninja", label: "NinjaTrader" },
        { v: "ibkr", label: "Interactive Brokers" },
      ],
    },
    {
      key: "style",
      q: "Preferred style?",
      options: [
        { v: "scalp", label: "Scalping (fast)" },
        { v: "swing", label: "Swing (multi-day)" },
        { v: "trend", label: "Trend-following" },
        { v: "mean", label: "Mean-reversion" },
      ],
    },
    {
      key: "markets",
    ...
    {
      key: "automation",
      q: "How automated do you want to be?",
      options: [
        { v: "alerts", label: "Signals/alerts only" },
        { v: "semi", label: "Semi-auto (confirm entries)" },
        { v: "full", label: "Fully automated" },
      ],
    },
    {
      key: "session",
      q: "When do you usually trade?",
      options: [
        { v: "lon", label: "London session" },
        { v: "ny", label: "New York session" },
        { v: "asia", label: "Asia session" },
        { v: "any", label: "Any / varies" },
      ],
    },
  ];
  ...
  const canContinue = useMemo(() => {
    ...
    if (s.multi) return Array.isArray(val) && val.length > 0;
    return Boolean(val);
  }, [answers, step]);
  ...
  const handleEmailSubmit = async (e) => {
    ...
    // TODO: Replace with your Airtable/HubSpot webhook URL
    const WEBHOOK_URL = ""; // e.g., "https://hooks.zapier.com/..." or Airtable automation endpoint
    if (WEBHOOK_URL) {
      try {
        ...
        console.warn("Webhook failed (continuing):", err);
      }
    }
  };
        ...
        {[
          {
            ...
            d: "We learn your goals, platform, markets and risk tolerance.",
          },
          {
            t: "Get a data-driven match",
            d: "We recommend strategies aligned to your inputs with transparent logic.",
          },
          {
            t: "Launch with controls",
            d: "Use presets or fine-tune risk. Alerts-only or full automation — your call.",
          },
        ].map((x, i) => (
        ...
        {[
          {
            ...
            a: "We score each algorithm across risk, style, markets, platform, and automation needs. The top scores are shown — you can still explore others.",
          },
          {
          ...
          },
          {
            ...
            a: "No. Installation is a guided flow and support is available 7 days a week.",
          },
          {
            q: "What if I’m not sure yet?",
            a: "Start with alerts-only mode, journal results, and switch to auto when comfortable.",
          },
        ].map((x, i) => (
          <div key={i} className="rounded-2xl p-5 border border-slate-200 bg-white shadow-sm">
...
const ALGORITHMS = [
  ...
  },
  {
    ...
    tags: {
      ...
      automation: ["alerts", "semi", "full"],
      experience: ["new", "intermediate"],
    },
  },
  {
    key: "vbb",
    name: "Volatility-Filtered BB",
    badge: "Mean-Rev • MT5",
...
function matchAlgos(answers) {
  ...
  const sessionAffinity = (algoKey, session) => {
    if (!session) return 0;
    if (algoKey === "trendpulse" && (session === "lon" || session === "ny")) return 0.8;
    if (algoKey === "swingx" && session === "any") return 0.5;
    return 0;
  };

  const marketSoftBonus = (algoMarkets = [], chosen = []) => {
    const overlap = chosen.filter((m) => algoMarkets.includes(m)).length;
  ...
  const results = ALGORITHMS.map((algo) => {
    ...
    if (answers.markets && answers.markets.length) {
      const mOverlap = answers.markets.filter((m) => algo.tags.markets.includes(m)).length;
      score += mOverlap * 8 * weights.markets;
